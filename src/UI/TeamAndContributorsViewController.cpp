...
DEFINE_TYPE(ScoreSaber::UI, TeamAndContributorsViewController);
...
{
    void TeamAndContributorsViewController::DidActivate(bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling)
    {
        if (firstActivation)
        {
            ...
            auto segmentedController = BeatSaberUI::CreateTextSegmentedControl(segmentedHorizontal->get_transform(), {0, 0}, {0, 0}, {}, std::bind(&TeamAndContributorsViewController::Show, this, std::placeholders::_1));
    ...
    void TeamAndContributorsViewController::Show(int idx)
    {
    ...
    UnityEngine::GameObject* TeamAndContributorsViewController::CreateCreditTab(int idx)
    {
    ...
    UnityEngine::GameObject* TeamAndContributorsViewController::CreateCreditTab(int idx)
    {
        ...
        for (int i = 0; i < members.size(); i++)
        {
            ...
            UIUtils::CreateTeamMemberLayout(layoutGroup->get_transform(), mem);
        }

        return scrollView->GetComponentInParent<HMUI::TextPageScrollView*>()->get_gameObject();
    }
    */
    /*
    UnityEngine::GameObject* TeamAndContributorsViewController::CreateCreditTab(int idx)
    {
        ...
        auto gridParent = CreateVerticalLayoutGroup(rootTab->get_transform());
        SetPreferredSize(gridParent, 115, -1);

        auto grid = CreateGridLayoutGroup(gridParent->get_transform());
        grid->set_cellSize(Vector2(45, 15));
        grid->set_spacing(Vector2(3, 3));
        grid->set_startAxis(GridLayoutGroup::Axis::Horizontal);
        grid->set_startCorner(GridLayoutGroup::Corner::UpperLeft);
